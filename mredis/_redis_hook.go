...
func (_this *BaseRedisClient) DialHook(next redis.DialHook) redis.DialHook {
	return next
}
...
func (_this *BaseRedisClient) ProcessHook(next redis.ProcessHook) redis.ProcessHook {
	return func(ctx context.Context, cmd redis.Cmder) error {
		_this.hookRedisCmder(cmd)
		return next(ctx, cmd)
	}
}
...
func (_this *BaseRedisClient) ProcessPipelineHook(next redis.ProcessPipelineHook) redis.ProcessPipelineHook {
	return func(ctx context.Context, cmd []redis.Cmder) error {
		for _, cmd := range cmd {
			_this.hookRedisCmder(cmd)
		}
...
func (_this *BaseRedisClient) hookRedisCmder(cmd redis.Cmder) {
	if len(cmd.Args()) > 1 {
		cmdName, ok := cmd.Args()[0].(string)
		if ok && commandsToPrefix[cmdName] {
			if key, ok := cmd.Args()[1].(string); ok {
				cmd.Args()[1] = _this.PrefixedKey(key)
			}
		}
	}
}
